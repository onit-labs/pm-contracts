...
contract InfiniteOutcomeDPMTokenTestMarketToken is OnitIODPMTestBase {
    function test_initialMarket_reverts_AlreadyInitialized() public {
        ...
        testMarket.initialize(defaultMarketConfig());
    }
    ...
    function test_initialMarket_reverts_BucketIdsNotStrictlyIncreasing() public {
        ...
        external
    {
        ...
        assertEq(market.getBalance(), initialBetValue, "market balance");
        assertEq(market.onitFactory(), address(factory), "onitFactory");
        assertEq(market.isResolver(RESOLVER_1), true, "resolver");

        // Check min/max bet sizes are set correctly
        assertEq(market.minBetSize(), MIN_BET_SIZE, "minBetSize");
        assertEq(market.maxBetSize(), MAX_BET_SIZE, "maxBetSize");
    ...
    function test_updateBettingCutoff() public {
        ...
        market.buyShares(alice, uint256(cost), DUMMY_BUCKET_IDS, DUMMY_SHARES);
    ...
    function test_voidMarket_reverts_NotOwner() public {
        market = newMarket(defaultMarketConfig());
